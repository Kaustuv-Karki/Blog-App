...
const SearchUsingCAtegory = () => {
  ...
  useEffect(() => {
    const fetchTags = async () => {
      const response = await fetch(`/api/blog/categoryList`, {
        method: "GET",
        headers: {
  ...
  const handleSearch = async (e) => {
    e.preventDefault();
    const category_query = selectedCategory.join("&");
    try {
      const response = await fetch(`/api/blog/category/${category_query}`, {
        method: "GET",
        headers: {
          "content-type": "application/json",
        },
  ...
  return (
    <div className="max-w-[1280px] mx-auto flex justify-center flex-col">
      ...
      <div className="flex gap-6 items-center max-w-[600px] mx-auto flex-wrap">
              ...
              <input
                ...
                onChange={(e) => {
                  if (e.target.checked) {
                    ...
                    setSelectedCategory(
                      selectedCategory.filter((tag) => tag !== e.target.value)
                    );
                  }
                }}
              />
